...
def evaluate_model(args):
    ...
    if args.load_checkpoint:
        checkpoint = torch.load(f"checkpoints/q2_2/checkpoint_{args.type}_5000.pth")
    ...
    for step in range(start_iter, max_iter):
        ...
        if (step % args.vis_freq) == 0:
            ...
            if args.type == "vox":
                ...
                rend = images_gt[0, ..., :3].detach().cpu().numpy().clip(0, 1)
                plt.imsave(f"results/q2/point_5000/q2_point_rgb_{step}.png", rend)
                pointclouds_gt = sample_points_from_meshes(mesh_gt, args.n_points).cpu()
                vis_point_cloud(
                    pointclouds_gt, f"results/q2/point_5000/q2_point_gt_{step}.gif"
                )
                vis_point_cloud(
                    predictions, f"results/q2/point_5000/q2_point_pred_{step}.gif"
                )
                mesh_gt = mesh_gt[0]
                vis_mesh(mesh_gt, f"results/q2/point_5000/q2_mesh_gt_{step}.gif")
